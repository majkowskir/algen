...
def f_Pokolenie(pula, output = False):
	...
	if iteracja >= Gen:
		...
		print("Pokolenie %s, \t średnia wartość f(x) w pokoleniu: %s"% (("{0:0=2d}".format(iteracja)), mean_y))

		max_y_x=max(ewaluacja_pokolenia,key=itemgetter(1))[0]
		max_y = math.log(max(ewaluacja_pokolenia,key=itemgetter(1))[1])

		print("Dla argumentu %s maksymalna wartość funkci wynosi: %s " % (round(max_y_x,d), round(max_y,d)))
...
def form_button():
    ...
    # warunek sumy prawdopodobienstw operacji Pc+Pm+Pk=1
    if (Pc+Pm)<=1:
